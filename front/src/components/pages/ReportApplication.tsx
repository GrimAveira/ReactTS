...
function ReportApplication() {
  ...
  const selectHandler = (newValue: { value: any }) => {
    setFilter((prev) => ({
      ...prev,
  ...
  const inputHandler = (event: { target: { name: any; value: any } }) => {
    setFilter((prev) => ({
      ...prev,
    ...
    <div className={styles.container}>
      <div className={styles.reports}>
        ...
        <Filters
          ...
          statuses={statuses}
        />
        {pagApps[counter].map((app: IApp) => (
          <ReportApplicationForm
            key={app.id}
            {...app}
            employees={employeesApp[app.id as keyof typeof employeesApp]}
            area={app.area}
            street={app.street}
            house={app.house}
